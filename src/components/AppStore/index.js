...
class AppStore extends Component {
  ...
  filterList = () => {
    ...
    const filter = appsList.filter(each => each.category === activeTab)
  ...
  render() {
    ...
    const getFilteredList = this.filterList()

    const searchList = getFilteredList.filter(each =>
        ...
        <ul className="tabs-list">
          {tabsList.map(each => (
            <TabItem
              ...
              activeTabItem={each.tabId === activeTab}
            />
          ))}
        </ul>
